...
it('can add the star name and star symbol properly', async() => {
    ...
    let user1 = accounts[1];
    let starId = 22;
    await instance.createStar('My First Star', starId, {from: user1});
    let contractName = await instance.name();
    let contractSymbol = await instance.symbol();
    assert.equal("Star Notary Tokens", contractName);
    assert.equal("SNT", contractSymbol);
});
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let starId1 = 77;
    await instance.createStar("User 1 Star", starId1, {from: user1});
    assert.equal(await instance.lookUptokenIdToStarInfo.call(starId1), "User 1 Star");
});